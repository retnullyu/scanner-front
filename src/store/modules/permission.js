    ...
    userRouter.forEach((item, index) => {
      if (item.resources === v.meta.resources) {
        if (item.children && item.children.length > 0) {
          v.children = recursionRouter(item.children, v.children) //递归子路由
        }
...
const permission = {
  ...
  mutations: {
    SET_ROUTERS: (state, routers) => {
      ...
      state.routers = constantRoutes.concat(routers) //拼接用户角色特定的路由
    }
  },
  actions: {
    GenerateRoutes({ commit }, data) {
      return new Promise(resolve => {
        commit('SET_ROUTERS', recursionRouter(data, asyncRouterMap)) //递归调用获取到的真实路由存入sate中
        resolve()
      })
    }
  }
