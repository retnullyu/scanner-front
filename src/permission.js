...
//全局前置守卫路由
//to 即将进入的路由对象 from 离开的路由对象 next 下一个钩子
router.beforeEach(async(to, from, next) => {   //router 跳转前都会判断
  // start progress bar
  NProgress.start()
  ...
  if (hasToken) {
    if (to.path === '/login') { //如果跳转登录界面且存在token直接跳转到首页
      next({ path: '/' })
      NProgress.done()
    } else {
      const relativePath = getRelativePath() //获取端口号之后的部分
